...
const timerHoursEl =
  document.querySelector('#timer-hours');
const timerMinutesEl = document.querySelector(
  '#timer-minutes',
);
const timerSecondsEl = document.querySelector(
  '#timer-seconds',
);
...
const startCountdown = () => {
  ...
  timerDaysEl.style.setProperty(
    '--percent',
    convertToPercent(days, 365),
  );
  timerHoursEl.style.setProperty(
    '--percent',
    convertToPercent(hours, 24),
  );
  timerMinutesEl.style.setProperty(
    '--percent',
    convertToPercent(minutes, 60),
  );
  timerSecondsEl.style.setProperty(
    '--percent',
    convertToPercent(seconds, 60),
  );
};
...
window.addEventListener('load', () => {
  ...
  const storedDate =
    JSON.parse(localStorage.getItem('countdown-date')) ||
    tomorrow;
  if (storedDate) {
